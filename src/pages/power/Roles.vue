  ...
  <div id="roles">
    ...
    <el-card>
        ...
        <el-table-column type="expand">
          <template slot-scope="{row}">
            ...
            <el-row v-for="(item1,i1) in row.children" :key="item1.id"
              :class="['tb-row',i1 == 0 ? 'td-row':'','verticalCenter','tagBox']">
              <!--渲染一级权限-->
              <el-col :span="5">
              ...
              <el-col :span="19">
                ...
                <el-row v-for="(item2,i2) in item1.children" :key="item2.id"
                  :class="[i2 === 0 ? '' : 'td-row','verticalCenter']">
                  <el-col :span="6">
                  ...
                  <el-col :span="18">
                    ...
                    <el-row>
                      <!--通过for循环嵌套渲染三级权限-->
                      <el-tag closable v-for="(item3,i3) in item2.children" type="warning" :key="item3.id" :span="3"
      ...
      <el-dialog title="添加角色" :visible.sync="addRoleDialogVisible" width="40%" @close="addRoleDialogClosed">
        <el-form :model="addRoleInfo" :rules="addRoleInfoRules" ref="addRoleInfoRef" label-width="80px">
          <el-form-item label="角色名称" prop="roleName">
      ...
      <el-dialog title="分配权限" :visible.sync="setRightsDialogVisible" width="40%" @close="setRightsDialogClosed">
        <!--树形控件-->
        <el-tree :data="treeRightsList" :props="rightsProps" show-checkbox node-key="id" default-expand-all
          :default-checked-keys="defaultChecked" ref="treeRightsRef"></el-tree>
        <!--对话框底部按钮-->
        <span slot="footer">
...
<script>
  export default {
    ...
    methods: {
      ...
      // 通过递归的方式将角色所有三级菜单的id保存到defaultChecked数值中
